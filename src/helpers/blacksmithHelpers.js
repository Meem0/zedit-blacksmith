module.exports = function({xelib, logger}) {
    ...
    blacksmithHelpers.numberToBytes = function(number) {
        if (typeof(number) === 'number' && number >= 0 && number <= 0xFFFFFFFF) {
            return number.toString(16).toUpperCase().padStart(8, '0').replace(/(..)(..)(..)(..)/, '$1 $2 $3 $4');
        }
    };
    ...
    blacksmithHelpers.getReferenceFromLongName = function(longName) {
        return xelib.WithHandle(
    ...
    blacksmithHelpers.getReferenceFromFormId = function(formId) {
        let formIdStr = typeof(formId) === 'number' ? formId.toString(16) : formId;
        if (!formIdStr || typeof(formIdStr) !== 'string') {
            return '';
        }

        if (Number.isNaN(Number('0x' + formIdStr))) {
            return '';
        }

        if (formIdStr.length > 8) {
            formIdStr = formIdStr.substring(formIdStr.length - 8);
        }
        else if (formIdStr.length < 8) {
            formIdStr = formIdStr.padStart(8, '0');
        }
        formIdStr = formIdStr.toUpperCase();

        const loadOrderString = formIdStr.substring(0, 2);
        const loadOrder = Number.parseInt(loadOrderString, 16);
        return xelib.WithHandle(xelib.FileByLoadOrder(loadOrder), fileId => {
            if (!fileId) {
                return '';
            }
            return xelib.WithHandle(xelib.GetElement(fileId, formIdStr), id => blacksmithHelpers.getReferenceFromRecord(id));
        });
    };
